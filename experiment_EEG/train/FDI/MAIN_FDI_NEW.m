...
hbar = 0.06;   Ts = 0.01;  num = 3;KT_FI = 23; threshold =1.5;target_array = [0, 1];
%数据的时间步设置
dt = 0.001; t = 0:dt:51.199; 
U = 1.1+2*sin(5*t)-2*cos(5*t); %% system input
 Tk = Ts/(t(2)-t(1)); steps = 0.999*size(t,2)/Tk; KT = 2.5/Ts; ep = 2/Ts+KT;
...
for k = 1:steps  
    ...
    error0_S(:,k) = sqrt(sum((XK_IDEN_raw(:,k)-model0_gen(:,k)).^2, 1));
    error1_S(:,k) = sqrt(sum((XK_IDEN_raw(:,k)-model1_gen(:,k)).^2, 1));
...
disp('块的识别结果（0或1）：');
disp(patterns);
accuracy = sum(patterns == y_test') / numel(patterns);
disp(accuracy);