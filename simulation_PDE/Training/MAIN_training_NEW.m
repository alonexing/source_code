...
% 3 input
c1 = 0:eta:1; c2 = 0:eta:1; c3 = 0:eta:1; c4 = -2:eta:4;
...
%%%%%%%%%%%%%%%% train
Ts = 0.01; Tk = Ts/(t(2)-t(1)); steps = size(t,2)/Tk; Timek = t(1,1:Tk/2:end); ktime = steps/1.2; 
...
%%%%%%%%%%%%%%%%%%% plot
    ...
    plot(Timek(1,1:2:2*Kp),xhk(2,1:Kp));
    plot(Timek(1,1:2:2*Kp),xk_iden(2,1:2:2*Kp));
    ...
    figure, box on,
    plot(W_hat_L2norm(1:300,1:5:end)');
    title('W_hat_L2norm');
end
...
WBar = [Weight_bar{1}, Weight_bar{2}, Weight_bar{3}, Weight_bar{4}];

% 用于保存mat文件
save D:\PDE_system\FDI_design_TRYnew.mat Weight_bar WBar center eta X_min X_max;
