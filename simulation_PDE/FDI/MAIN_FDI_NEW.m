...
load D:\PDE_system\FDI_design_TRYnew.mat;
load D:\PDE_system\testdata_new.mat;

%% %%%%%%%%%%%%%%%%%% initial condition & parameters

rho_int = 0;      unknown_mode = 9;% 未知模式的编号
a = 2; A = [a;a;a]; %% parameters of threshold
%KT：求误差的滑动时间窗，KT_FI：防止误判的时间窗，threshold：未知模式的误差阈值，target_array：测试数据的模式顺序
hbar = 0.06;   Ts = 0.01;  num = 3;KT_FI = 23; threshold =1.5;target_array = [1, 2, 3, 4, unknown_mode];
%数据的时间步设置
 Tk = Ts/(t(2)-t(1)); steps = 1*size(t,2)/Tk; KT = 1.8/Ts; ep = 2/Ts+KT;
Timek = t(1,1:Tk/2:end); xk_iden = test_red(1:Tk/2:end,:)'; NNinputk = [test_red(1:Tk/2:end,:), U(1,1:Tk/2:end)']'; 
phi = sqrt(2/pi)*sin(z'*[1,2,3]); %% n_z*3
dz = z(2)-z(1);
z_num = size(z,2);
% 按同样时间步采样的原始数据x(z,t)
...
%% %%%%%%%%%%%%%%%%%% FDI 
FI_state = repmat(xk_iden(1:3,1),4,1);
recognized_time = []; 
recognized_values = [];
current_state = NaN;
...
for k = 1:steps  
    ...
    % 重建回xbar(z,t)
    [model0_gen(:,k)] = KLD_construct(FI_state(1:3,:)', num, s_eigvector);
    [model1_gen(:,k)] = KLD_construct(FI_state(4:6,:)', num, s_eigvector);
    ...
    error0_S(:,k) = sqrt(sum((XK_IDEN_raw(:,k)-model0_gen(:,k)).^2, 1) * dz);
    error1_S(:,k) = sqrt(sum((XK_IDEN_raw(:,k)-model1_gen(:,k)).^2, 1) * dz);
    ...
    % FI_decision为每一步的识别结果，recognized_values为识别到的模式切换结果，recognized_time为识别到的模式切换时间
        all_error = [error0_T(:,k);error1_T(:,k);error2_T(:,k);error3_T(:,k)];
...
if isequal(recognized_values(1:5), target_array)  % 如果识别出的模式与测试数据模式相同，则认为成功
    disp('识别成功');
else
    disp('识别失败');
end
...
% 故障2模式辨识器
plot(error2_T', 'b-', 'DisplayName', 'Fault2'); 

% 故障3模式辨识器
plot(error3_T', 'm--', 'DisplayName', 'Fault3'); 
